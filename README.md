...
Infer what each model does (classification, regression, task type)
Estimate how good it is (accuracy, reliability)
Detect weaknesses (edge cases, failures, noise sensitivity)
Show your work (confidence backed by actual probe counts, not speculation)
...
Shape matching (free) — Feature count → likely task
Reference comparison (30 probes) — Send same inputs to unknown + reference
Edge case testing (20 probes) — Detect failures
Confidence scoring (formula) — Confidence = evidence invested
5. Start Profiling
...
Your Code (To Build)
profiler.py — Main script that profiles all 13 models
profiles.json — Output file with all profiles
dashboard.html — Web UI that loads profiles.json
...
✅ A probe strategy under budget constraints
✅ A confidence scoring system tied to evidence
✅ A dashboard that displays pre-computed results
✅ A workflow for characterizing black-box models
You'll learn:
...
Print CHEAT_SHEET.md and keep it on your desk
Fill in credentials in starter_kit.py
Run starter_kit.py to confirm API access
Skim Phase 0-1 of hackathon_concrete_plan.md
Assign team roles: Backend, Frontend, Strategy, DevOps
Start profiling using starter_code_snippets.py
